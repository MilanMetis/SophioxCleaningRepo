...
def is_partial_row(row):
    ...
    """
    has_date = any(
        'date' in col.lower() and pd.notnull(row[col]) and str(row[col]).strip()
    ...
    return not has_date and not has_amount and has_narration
...
def clean_debit_credit(df):
    ...
    has_drcr = any(
        re.fullmatch(r'(?:dr[/|]cr|cr[/|]dr|dricr|dr_cr|drcr|dr\.|cr\.|cr/dr)', col, flags=re.IGNORECASE)
        for col in df.columns
    ) or any(col in ['type', 'txn type', 'transaction type', 'cr/dr'] for col in cols_lower)


    has_mixed_amount = any(
        re.search(r'debit.*credit|dr.*cr|amount', col, re.IGNORECASE) for col in df.columns
    )
...
def split_drcr_from_amount_column(df):
    """
    Handles case where a single Amount column contains values like:
    150.00Dr / 1000.00Cr
    """

    # Run only if Debits & Credits are missing
    ...
    amount_col = None
    for col in df.columns:
        if re.search(r'amount|amt', col.lower()):
            amount_col = col
            break
...
def normalize_drcr_value(val):
    """
    Normalize DR/CR values to standard format
    """
...
def parse_debit_credit_split_safe(df):
    ...
    drcr_cols = [
        ...
        if re.fullmatch(
            r'(?:DR[/|]CR|CR[/|]DR|DRICR|DR[_]CR|DRCR|DR\.?|CR\.?)',
            col.strip(),
            flags=re.IGNORECASE
        )
    ]
...
def normalize_headers(df):
    ...
    """
    headers = {
        "XN Date": {"Date(ValueDate)","Tran Date","Date Day/Night","TransactionDate","TxnDate& Time","DAT VALUE", "Date(Value Date","Date& Time", "Post Date", "PostDate","Txn Posted Date", "TRANSACTION DATE", "Tran Date", "TranDate","XN Date", "Date", "Transaction date", "Txn Date", "Post date","ate", "DATE", "Transaction Date", "TRAN DATE", "TRANDATE","Transactio n Date"},
    ...
    HEADER_REGEX = {
        "XN Date": [
            ...
            r'\bdate\s*value\s*date\b',r'\btran\s*date\b',
    ...
    if "Debits" not in df.columns and "Credits" not in df.columns:
        for col in df.columns:
            if df[col].astype(str).str.contains(r'\d{1,3}[.,]\d{2,3}', regex=True).any():
                if "Debits" not in df.columns:
                    df["Debits"] = df[col].apply(lambda x: extract_amount(x, 'dr'))
                if "Credits" not in df.columns:
                    df["Credits"] = df[col].apply(lambda x: extract_amount(x, 'cr'))
...
def clean_bank_statement(df, file_path=None, logging=True):
    ...
    def step_remove_metadata_rows(df):
        """
        Remove metadata rows like BROUGHTFORWARD, CLOSING BALANCE, etc.
        """

        metadata_keywords = [
            "BROOGHTFORWARD", "CLOSING BALANCE", "TRANSACTIONTOTAI",
            "RROUGHTFOROWARD", "BROUGHTFORWARD", "TOTAL",
            "YOUR OPENING", "BALANCE ON", "BROUGHTFORWARDED",
            "CLOSINGBALANCE", "OPENING BALANCE", "OPENING BALANCE:",
            "TRANSACTIONTOTAL CLOSING BALANCE",
            "TRANSACTION TOTAL DRICR","LOSINGBALANCE",
            "CLOSING BALANCE :",
            "TOTALDEBIT AMOUNT", "TOTAL CRIGHT AMOUNT",
            "B/F", "BROUGHT FORWARD"
        ]
        ...
        def is_metadata_row(row):
            ...
            xn_date = str(row.get("XN Date", "")).strip()

            # normalize narration
            norm_narration = re.sub(r'[^A-Z]', '', narration.upper())

            # rule 1: narration must exactly match metadata keyword
            if norm_narration in normalized_keywords:
                return True

            # rule 2: no date + narration looks like metadata
            if not xn_date or xn_date.lower() in ["nan", "none", ""]:
                if norm_narration in normalized_keywords:
                    return True
...
#     input_csv = r"c:\Users\admin\Downloads\cleaning_code\download 392 P VIJAYAKUMAR.csv"
#     output_csv = r"c:\Users\admin\Downloads\cleaning_code\download 392 P VIJAYAKUMAR_cleaned.csv"
