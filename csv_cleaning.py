...
def drop_last_rows(df):
	...
	row_prev = row - 1
	row_for = row + 1
	base_shape_row = get_shape(rows[row])

	if row-1 == 0:
		base_shape_row_prev = get_shape(rows[row-1])
	else:
		base_shape_row_prev = 0

	if row+1 < len(df):
		base_shape_row_for = get_shape(rows[row+1])
	else:		
	...
	# cleaned_df.to_csv("debug_after_drop_last_rows.csv", index=False)  # Debug output
...
def parse_balance(value):
	...
	is_cr = "CR" in text
		...
		# Check for parentheses containing numeric value (indicates negative balance)
	has_valid_parentheses = False
	if '(' in text and ')' in text:
		# Extract content inside the first pair of parentheses
		match = re.search(r'\((.*?)\)', text)
		if match:
			inner = match.group(1)
			# Remove commas and spaces, then check if at least one digit exists
			inner_clean = re.sub(r'[,\s]', '', inner)
			if re.search(r'\d', inner_clean):
				has_valid_parentheses = True
	...
	try:
		...
		# Balance sign logic (ONLY here)
				# Balance sign logic (ONLY here)
		if has_minus or has_valid_parentheses:
			return -abs(amount)  # Explicit minus or valid parentheses → negative absolute
		elif is_dr:
			return -amount
		else:
